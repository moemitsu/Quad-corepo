...
@pytest.mark.asyncio
async def test_names(async_client):
    print('------------------------------test_names')
    Firebase1 = os.getenv('FIREBASE_UID1')
    ...
    response = await async_client.get("/api/v1/names", headers=headers)
    print(response)
    assert response.status_code == starlette.status.HTTP_200_OK
    response_obj = response.json()
    print(response_obj)
    assert response_obj['adult_names'] == adult_names
    assert response_obj['child_names'] == child_names